...
import type { CompetitorData } from './mock-scrape';
...
class SimpleSEOComparer {
  ...
  async loadOurSiteData(url: string = 'https://www.chiangmaiusedcar.com'): Promise<CompetitorData> {
    ...
    if (fs.existsSync(scrapePath)) {
      ...
      if (files.length > 0) {
        ...
        if (latestFile) {
          const filePath = path.join(scrapePath, latestFile);
          const data: CompetitorData[] = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
  ...
  async compareWithCompetitors(): Promise<SEOComparison> {
    // Generate mock data if needed
    const { MockCompetitorScraper } = await import('./mock-scrape');
    const scraper = new MockCompetitorScraper();
    const competitorData = await scraper.scrapeAllCompetitors();

    // Save the data
    const timestamp = new Date().toISOString().split('T')[0];
    MockCompetitorScraper.saveResults(competitorData, `competitor-data-${timestamp}.json`);
  ...
  private generateRecommendations(
    ...
    const allCompetitorKeywords = analyses.flatMap(a =>
      a.keywordGaps
        .filter(gap => gap.startsWith('They target:'))
        .map(gap => gap.replace('They target: ', ''))
    );
  ...
  static saveComparison(comparison: SEOComparison, filename?: string): void {
    ...
    const summary = SimpleSEOComparer.generateTextSummary(comparison);
...
export { SimpleSEOComparer, type SEOComparison, type CompetitorAnalysis };
...
if (require.main === module) {
  async function main() {
    ...
    try {
      ...
      // Save results
      SimpleSEOComparer.saveComparison(comparison);
