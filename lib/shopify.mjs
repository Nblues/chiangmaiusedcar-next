...
let __kvPromise = null;
const hasEnvValue = v => v != null && String(v).trim() !== '' && String(v) !== 'undefined';
const isKvConfigured = () => {
  // Vercel KV uses Upstash REST under the hood; these vars are set when KV is connected.
  const url =
  ...
  return hasEnvValue(url) && hasEnvValue(token);
};
...
export async function getAllCars() {
  ...
  const now = Date.now();
  const ttlMs = process.env.NODE_ENV === 'development' ? 15_000 : 120_000;
  if (
  ...
  // Tier-2 cache (production): Vercel KV. Helps when serverless instances are cold
  // and in-memory cache isn't available.
  const storeKey = getShopifyStoreKey() || 'unknown-store';
  const kvCacheKey = `cache:allcars:v1:${storeKey}`;
  const kvTtlSeconds = 600; // 10 minutes: statuses are refreshed client-side

  const shouldUseLocalFileCache =
    process.env.NODE_ENV !== 'development' &&
    // Avoid filesystem writes on Vercel serverless
    !process.env.VERCEL;

  const localFileCachePath = (() => {
    try {
      // Prefer Next's build cache folder when available.
      // Works for `next start` (local Lighthouse) and most non-serverless Node deployments.
      return `${process.cwd().replace(/\\/g, '/')}/.next/cache/cnx-allcars.v1.json`;
    } catch {
      return null;
    }
  })();
  ...
  if (shouldUseLocalFileCache && localFileCachePath) {
    try {
      const fs = await import('node:fs/promises');
      const stat = await fs.stat(localFileCachePath);
      const fileTtlMs = 10 * 60 * 1000;
      if (now - stat.mtimeMs < fileTtlMs) {
        const raw = await fs.readFile(localFileCachePath, 'utf8');
    ...
    promise: fetchAndParse()
      .then(async list => {
        ...
        if (shouldUseLocalFileCache && localFileCachePath) {
          try {
            const fs = await import('node:fs/promises');
            const dir = localFileCachePath.split('/').slice(0, -1).join('/');
            await fs.mkdir(dir, { recursive: true });
            await fs.writeFile(localFileCachePath, JSON.stringify(list), 'utf8');
          } catch {
