...
export async function getAllCars() {
  ...
  const now = Date.now();
  const ttlMs = process.env.NODE_ENV === 'development' ? 0 : 900000; // 5 mins in dev, 15 mins prod
  if (
  ...
  const shouldUseLocalFileCache = !process.env.VERCEL && process.env.NODE_ENV !== 'development';
...
export async function getHomepageCars(limit = 8) {
  const now = Date.now();
  const ttlMs = process.env.NODE_ENV === 'development' ? 0 : 900000;
  const limitNum = (() => {
...
export async function getBrandCounts() {
  const now = Date.now();
  const ttlMs = process.env.NODE_ENV === 'development' ? 0 : 30 * 60 * 1000;
  if (
...
async function getCarByHandleUncached(handle) {
  const metafieldRefSelection = `
            reference {
              __typename
              ... on Metaobject {
...
export async function getCarByHandle(handle) {
  // Cache layer for GraphQL fetching by handle (prevents spamming Shopify from multi-component SSG)
  globalThis.__CNX_CAR_CACHE__ = globalThis.__CNX_CAR_CACHE__ || {};
  const now = Date.now();
  // 5 minutes in Dev, 15 minutes in Prod
  const ttlMs = process.env.NODE_ENV === 'development' ? 0 : 900000;
  
  const cacheEntry = globalThis.__CNX_CAR_CACHE__[handle];
  if (cacheEntry?.data && now - cacheEntry.ts < ttlMs) {
  ...
  const promise = getCarByHandleUncached(handle);
  globalThis.__CNX_CAR_CACHE__[handle] = { promise, ts: now };
  
  try {
    const data = await promise;
    globalThis.__CNX_CAR_CACHE__[handle] = { data, ts: Date.now() };
    return data;
  } catch (err) {
    delete globalThis.__CNX_CAR_CACHE__[handle];
    throw err;
  }
}
/ /   F o r c e   r e b u i l d   f o r   c a c h e   f l u s h  
 