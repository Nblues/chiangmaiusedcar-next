...
// or exposed only a subset (e.g., read-only token).
const normalizeKvEnv = () => {
  const pairs = [
  ...
  // Best-effort: if a read-only token exists but the main token is missing, allow reads.
  // Writes will be attempted but safely caught by existing try/catch.
...
export async function getHomepageCars(limit = 8) {
  ...
  globalThis.__CNX_HOMEPAGE_CARS_CACHE__ = {
    ...
    promise: fetchAndParse()
      .then(list => {
        globalThis.__CNX_HOMEPAGE_CARS_CACHE__ = { key: cacheKey, ts: Date.now(), data: list, promise: null };
        return list;
      })
      .catch(error => {
        // eslint-disable-next-line no-console
        console.error('getHomepageCars error:', error);
        globalThis.__CNX_HOMEPAGE_CARS_CACHE__ = { key: cacheKey, ts: Date.now(), data: null, promise: null };
        return [];
      }),
  };
