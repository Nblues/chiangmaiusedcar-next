...
export const testSocialSharing = async url => {
  try {
    // Import fetchWithTimeout for external API calls
    const { fetchWithTimeout } = await import('../src/lib/fetchWithTimeout.ts');

    // Test Facebook debugger with timeout and fallback
    let fbResponse;
    try {
      fbResponse = await fetchWithTimeout(
      ...
      console.error('Facebook API test error:', error.message);
      fbResponse = { ok: false };
    }
