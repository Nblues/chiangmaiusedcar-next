 ...
 */
export function parseCarTitle(title) {
  const parsed = {
    brand: null,
    model: null,
    year: null,
  ...
  const brands = [
    ...
    'ISUZU', 'FORD', 'CHEVROLET', 'HYUNDAI', 'KIA',
  ...
  } else if (titleUpper.includes('MANUAL') || titleUpper.includes('MT') || titleUpper.includes('เกียร์ธรรมดา')) {
    parsed.transmission = 'เกียร์ธรรมดา';
  }

  // Extract model (simplified)
  const modelPatterns = [
    'CAMRY', 'COROLLA', 'VIOS', 'YARIS', 'FORTUNER', 'HILUX', 'REVO',
    'CIVIC', 'CITY', 'ACCORD', 'CR-V', 'HR-V', 'JAZZ',
    'CX-3', 'CX-5', 'CX-7', 'MAZDA2', 'MAZDA3',
    'D-MAX', 'MU-X', 'MU-7',
    'RANGER', 'EVEREST', 'FOCUS',
    'PAJERO', 'TRITON', 'LANCER',
 ...
 * @returns {Object} - Additional car info
 */
export function parseCarDescription(description) {
  const parsed = {
    mileage: null,
    price_info: null,
    down_payment: null,
...
export function parseCarData(product) {
  ...
  return {
    ...
    // Parsed car data
    brand: titleData.brand || (product.vendor && product.vendor !== 'ครูหนึ่งรถสวย' ? product.vendor.toLowerCase() : null),
