...
function cleanMemoryStore() {
  if (memoryStore.size <= MAX_MEMORY_STORE_SIZE) return;
  const now = Date.now();
  ...
  // If still too large, drop the first (oldest) 20%
  if (memoryStore.size > MAX_MEMORY_STORE_SIZE) {
    let toDelete = Math.floor(memoryStore.size * 0.2);
...
export async function rateLimit(key, options = {}) {
  ...
  try {
    ...
    const count = await kvClient.incr(bucketKey);
    if (count === 1) {
      await kvClient.expire(bucketKey, Math.ceil(windowMs / 1000));
    }
