...
// Lazy KV loader — prevent crash when @vercel/kv is not installed or not configured
// Same pattern as shopify.mjs to be safe on local dev without KV creds
let __kv = null;
let __kvLoaded = false;

async function getKv() {
  if (__kvLoaded) return __kv;
  __kvLoaded = true;
  try {
...
async function readStatusMapFromKV() {
  ...
  statusMapInFlight = (async () => {
    ...
    if (!kvClient) throw new Error('KV client unavailable');
    const data = await kvClient.get(CAR_STATUSES_MAP_KEY);
...
async function writeStatusMapToKV(map) {
  ...
  const kvClient = await getKv();
  if (!kvClient) throw new Error('KV client unavailable');
  await kvClient.set(CAR_STATUSES_MAP_KEY, safe);
...
export async function readCarStatuses() {
  ...
  try {
    ...
    Object.entries(statuses).forEach(([carId, record]) => {
      if (record) setMemoryCache(carId, record);
    });
...
export async function readCarStatus(carId) {
  ...
  try {
    ...
    if (record) {
      setMemoryCache(String(carId), record);
      return record;
    }
    ...
    const kvClient = await getKv();
    const legacy = kvClient ? await kvClient.get(legacyKey) : null;
    if (legacy && typeof legacy === 'object') {
      const next = { ...map, [String(carId)]: legacy };
      await writeStatusMapToKV(next);
      setMemoryCache(String(carId), legacy);
      return legacy;
    }
  ...
  try {
    ...
    if (missing.length > 0) {
      ...
      const legacyValues = await Promise.all(
        missing.map(id =>
          kvClient ? kvClient.get(`${CAR_STATUS_PREFIX}${id}`).catch(() => null) : null
        )
      );
...
export async function writeCarStatuses(statuses) {
  try {
    ...
    if (isPlainObject(statuses)) {
      Object.entries(statuses).forEach(([carId, record]) => {
        if (record) setMemoryCache(String(carId), record);
      });
    }
    return 'vercel-kv';
  } catch (error) {
...
export async function updateCarStatus(carId, status) {
  ...
  setMemoryCache(String(carId), statusRecord);
