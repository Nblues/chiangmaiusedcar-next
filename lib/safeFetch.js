 ...
 */
async function safeGraphQLFetch(endpoint, query, variables = {}, options = {}) {
 ...
 */
async function safeExternalFetch(url, options = {}) {
  return safeFetch(url, {
    timeout: 15000,
    retries: 1,
    retryDelay: 2000,
 ...
 */
async function safeImageFetch(imageUrl, options = {}) {
 ...
 */
function safeFormatPrice(amount, currency = 'THB', fallback = 'ติดต่อสอบถาม') {
  try {
 ...
 */
function safeLocalStorage(key, fallback = null) {
  try {
 ...
 */
function safeObjectKeys(obj, fallback = []) {
  try {
    if (obj === null || obj === undefined || typeof obj !== 'object') {
      return fallback;
    }
 ...
 */
function safeObjectValues(obj, fallback = []) {
  try {
    if (obj === null || obj === undefined || typeof obj !== 'object') {
      return fallback;
    }
 ...
 */
function safeObjectEntries(obj, fallback = []) {
  try {
    if (obj === null || obj === undefined || typeof obj !== 'object') {
      return fallback;
    }
