...
export function initPerformanceObserver() {
  ...
  try {
    // Observe navigation timing only (not per-resource — too many requests)
    ...
    // NOTE: Resource-level observer intentionally removed.
    // Tracking every .js/.css/.webp file per page caused dozens of simultaneous
    // POST /api/analytics requests, crashing into the 429 rate-limit on every load.
  } catch (error) {
...
export function reportMetrics(metrics) {
  ...
  if (navigator.sendBeacon) {
    ...
    // Fallback to fetch — silently ignore 429 rate-limit responses
    fetch(MONITORING_CONFIG.endpoint, {
      ...
      .then(res => {
        if (!res.ok && res.status !== 429 && MONITORING_CONFIG.debug) {
          console.warn('Analytics responded with', res.status);
        }
      })
      .catch(error => {
