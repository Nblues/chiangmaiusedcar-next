...
// 1. getAllCars
c = c.replace(/const getAny = keys => \{\s*for \(const k of keys\) \{\s*if \(hasValue\(mergedSpec\[k\]\)\) return mergedSpec\[k\];\s*\}\s*return null;\s*\};\s*\/\/ Prefer Shopify metafields[\s\S]*?bodyType:\s*'ประเภทตัวถัง',\s*\]\);/m, 
...
c = c.replace(/const getAny = keys => \{\s*for \(const k of keys\) \{\s*if \(hasValue\(mergedSpec\[k\]\)\) return mergedSpec\[k\];\s*\}\s*return null;\s*\};\s*const parsed = parseCarData\(\{[\s\S]*?const model = getAny\(\['model', 'รุ่น'\]\);\s*if \(hasValue\(brand\)\) payload\.brand = brand;\s*if \(hasValue\(model\)\) payload\.model = model;/m, 
...
// 4. getCarByHandle
c = c.replace(/const getAny = keys => \{\s*for \(const k of keys\) \{\s*if \(hasValue\(mergedSpec\[k\]\)\) return mergedSpec\[k\];\s*\}\s*return null;\s*\};\s*\/\/ Strict: show specs ONLY[\s\S]*?bodyType:\s*'ประเภทตัวถัง',\s*\]\);/m, 
`const { year, mileage, transmission, drivetrain, fuelType, brand, model, installment, category, bodyType } = normalizeCarSpec(mergedSpec, parsed, n.vendor);`);
