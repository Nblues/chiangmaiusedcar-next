...
import { fetchBrands, fetchModels } from '../../lib/carFormActions';
import { saveValuation, getRecentValuations, deleteValuation, updateValuation } from '../../lib/valuationActions';
...
export default function ValuationDashboard() {
  ...
  const handleFetchSubModels = async () => {
    ...
    const raw = await fetch(`/api/admin/evaluate?action=getModels&brand=${formData.brand}&model=${formData.model}&year=${formData.year}`); const res = await raw.json();
    if (res.success && res.data) {
      setSubModelsList(res.data);
      if (res.data.length > 0) {
        setFormData(prev => ({ ...prev, subModel: (res.data && res.data[0]) ? res.data[0] : '' }));
      }
  ...
  const handleAnalyze = async () => {
    setLoading(true);
    const raw = await fetch(/api/admin/evaluate, {method:POST, headers:{Content-Type:pplication/json}, body:JSON.stringify(formData)}); const res = await raw.json();
    if (res.success) {
      setResult(res.data);
    } else {
                    ...
                    <Card className="shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-none rounded-3xl bg-white overflow-hidden relative">
                      ...
                      <CardContent>
                        <ul className="space-y-3">
                          {result.pros?.map((pro: any, idx: number) => (
                            <li
                              key={idx}
                        ...
                        <div className="mt-6 border-t border-slate-100 pt-4">
                          ...
                          <ul className="space-y-3">
                            {result.cons?.map((con: any, idx: number) => (
                              <li
                                key={idx}
                                className="flex items-start text-sm text-slate-700 font-medium leading-relaxed"
                              >
