...
async function getTTBSubModels(brand: string, model: string, year: number): Promise<{ success: boolean, data?: string[], error?: string }> {
  try {
    const searchUrl = `https://www.roddonjai.com/service/bluebook-search-result?carBrand=${encodeURIComponent(brand)}&carModel=${encodeURIComponent(model)}&year=${year}`;
    const res = await fetch(searchUrl, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0' },
...
async function evaluateCarWithAI(formData: {
  brand: string;
  model: string;
  subModel?: string;
  bodyType?: string;
...
export default async function handler(req, res) { 
  if(req.method === 'GET') { 
    if(req.query.action === 'getModels') {
      const models = await getTTBSubModels(req.query.brand, req.query.model, req.query.year);
      return res.status(200).json(models);
    }
  }
  if(req.method === 'POST') {
    const r = await evaluateCarWithAI(req.body);
