...
function useAdminAuth() {
  ...
  useEffect(() => {
    ...
    const checkBasicAccess = () => {
      // ตรวจสอบ localStorage สำหรับผู้ที่ผ่านการยืนยันแล้ว
      const savedAccess = localStorage.getItem('admin_access_granted');
      const lastAccess = localStorage.getItem('admin_last_access');

      // ตรวจสอบว่าเข้าถึงภายใน 24 ชั่วโมง
      if (savedAccess === 'true' && lastAccess) {
        const lastAccessTime = new Date(lastAccess);
        const now = new Date();
        const hoursDiff = (now - lastAccessTime) / (1000 * 60 * 60);
      ...
      if (admin_access === 'true' || secret === validSecret || key === 'admin') {
        setHasAccess(true);
